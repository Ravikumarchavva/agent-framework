...
async def demo_mailbox_backpressure() -> None:
    ...
    # Fill to capacity
    for i in range(3):
        env = Envelope(sender=sender, target=target, content=[TextBlock(text=f"msg-{i}")])
        mbox.put_nowait(env)

    print(f"  Filled to capacity=3")

    # 4th message → MailboxFullError (public exception, not private state)
    try:
        overflow = Envelope(sender=sender, target=target, content=[TextBlock(text="overflow")])
    ...
    mbox.put_nowait(Envelope(sender=sender, target=target, content=[TextBlock(text="retry-ok")]))
    print(f"  Re-enqueue after drain: succeeded")
...
class OrderingAgent(RuntimeAgent):
...
class CountingAgent(RuntimeAgent):
...
async def demo_timeout() -> None:
    ...
    class StalledAgent(RuntimeAgent):
        async def on_message(
            self, ctx: MessageContext, content: list[ContentBlock]
        ) -> object:
...
async def demo_fanout_isolation(runtime: LocalRuntime) -> None:
    ...
    class GoodListener(RuntimeAgent):
        async def on_message(
            self, ctx: MessageContext, content: list[ContentBlock]
        ) -> object:
    ...
    class BadListener(RuntimeAgent):
        async def on_message(
            self, ctx: MessageContext, content: list[ContentBlock]
        ) -> object:
    ...
    print(f"  bad_listener crashed but good_listener unaffected ✓")
