...
import asyncio
from agent_substrate.capabilities.tools.web_surfer.tool import WebSurferTool
import time
...
async def main():
    ...
    # Initialize tool (set headless=True for headless execution in CI/headless environments)
    ...
    try:
        ...
        if len(result.content) > 1 and result.content[1].type == "image":
            print(
                f"✅ Screenshot captured: {len(result.content[1].data)} bytes (base64)\n"
            )
        ...
        result = await surfer.execute(
            action="execute_js", javascript="document.querySelectorAll('a').length"
        )
        print(f"✅ {result.content[0].text}\n")
