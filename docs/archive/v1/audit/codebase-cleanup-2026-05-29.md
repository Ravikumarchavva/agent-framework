...
Rename `server/context.py` → `server/dependencies.py` and rename the class `ServerContext` → `ServerDependencies` (or `AppState`). Update all `from ravi.server.context import ServerContext` imports in routes and lifespan.
