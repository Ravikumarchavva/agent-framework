...
| `Supervisor` (production `SupervisorProtocol`: durable spawn tree + idempotent spawn, `ravi_run_tree`/`ravi_spawn_effects`) | `infrastructure/runtime/supervisor.py` |
...
!!! note "Durable multi-run coordination (production)"
    ...
    *runs* together durably — so a spawn/join tree, and cross-run cancel
    cascades, survive a worker crash — is `Supervisor` in
