...
| Date | Summary |
|---|---|
| [2026-07-02](2026-07-02-system-audit.md) | First full system audit (3 parallel deep-dives: runtime execution path, serving/state, orchestration/messaging). Verdict: single-run durability is real; the coordination layer (SignalBus, Supervisor, HITL, cancel, locks, fanout) is in-process memory even in Postgres mode — blocks horizontal scaling AND restart-safety. Cross-tenant IDOR found. Budgets/deadlines/fairness/agent-loop-tracing are dead or unwired code. Microservices are scaffolding (human_gate has zero callers). Led directly to the approved remediation program (see plan / roadmap). |
