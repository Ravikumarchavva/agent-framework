...
from ravi.core.messages.content import DocumentBlock, CodeBlock, DataBlock, ErrorBlock, TextBlock
...
def test_document_content_validation_and_coercion() -> None:
    doc = DocumentContent(data=b"fake-pdf-data", media_type="application/pdf", filename="invoice.pdf")
    ...
    message_dict = UserMessage(content=[
        "Read this:",
        {
            "type": "document",
...
def test_document_block_extraction_from_tool_result() -> None:
    ...
    tool_result = ToolResult(
        content=[
            ...
            DocumentBlock(data=pdf_b64, media_type="application/pdf", filename="report.pdf"),
        ]
    )
...
def test_document_content_serialization_anthropic() -> None:
    doc = DocumentContent(data=b"pdf-bytes", media_type="application/pdf", filename="test.pdf")
...
def test_block_text_upgraded_fallbacks() -> None:
    ...
    doc = DocumentBlock(data="base64...", media_type="application/pdf", filename="invoice.pdf")
