...
def test_text_and_reasoning_deltas() -> None:
    ...
    assert map_kernel_event(ReasoningDelta(text="hmm")) == ReasoningDeltaEvent(text="hmm")


def test_tool_call_progress_maps_with_agent_and_depth() -> None:
    ev = AgentProgress(
        agent_id=_AID, step=AgentStep.TOOL_CALL, content="web_search",
...
def test_tool_result_ok_and_error() -> None:
    ok = map_kernel_event(AgentProgress(
        agent_id=_AID, step=AgentStep.TOOL_RESULT, content="web_search: ok",
        run_id="r", depth=1, metadata={"call_id": "c1"},
    ))
    assert isinstance(ok, ToolResultEvent) and ok.ok is True and ok.tool_name == "web_search"

    err = map_kernel_event(AgentProgress(
        agent_id=_AID, step=AgentStep.TOOL_RESULT, content="web_search: error",
        run_id="r", depth=1, metadata={"call_id": "c1"},
    ))
...
def test_handoff_tool_call_becomes_handoff_event() -> None:
    ev = AgentProgress(
        ...
        step=AgentStep.TOOL_CALL, content="handoff_researcher", run_id="r", depth=0,
    )
    wire = map_kernel_event(ev)
...
def test_handoff_tool_result_is_dropped() -> None:
    ev = AgentProgress(
        agent_id=AgentId(type="assistant", key="coordinator"),
        step=AgentStep.TOOL_RESULT, content="handoff_researcher: ok", run_id="r",
    )
    assert map_kernel_event(ev) is None


def test_completion_extracts_text_and_tool_calls() -> None:
    ev = CompletionEvent(content=[
        TextBlock(text="The answer is "),
        TextBlock(text="42"),
        ToolUseBlock(call_id="t1", tool_name="calculator", arguments={"expr": "6*7"}),
    ])
    wire = map_kernel_event(ev)
...
def test_thinking_and_streamdone_are_dropped() -> None:
    thinking = AgentProgress(agent_id=_AID, step=AgentStep.THINKING, content="step 1", run_id="r")
