...
from raavan.core.pipelines.schema import PipelineConfig
from raavan.server.models import Pipeline, PipelineRun

logger = logging.getLogger("raavan.server.routes.builder")
...
async def run_pipeline(
    ...
    async def sse_stream():
        ...
        try:
            ...
            # Execute the pipeline — use Any to dispatch via duck-typing
            ...
            if hasattr(_runnable, "run_stream"):
                ...
                # Fallback run
                result = await _runnable.run(body.input_text)
                output_val = (
                    result.output
                    if isinstance(result.output, str)
                    else "".join(str(p) for p in result.output)
                )
                yield _sse_event("text_delta", {"content": output_val})
