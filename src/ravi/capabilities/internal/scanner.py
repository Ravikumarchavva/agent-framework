...
class CatalogScanner:
    ...
    def _load_skill_metadata(self, skill_dir: Path, skill_md: Path) -> Optional[Any]:
        ...
        try:
            from ravi.capabilities.internal.skill_loader import (
                SkillLoader,
            )
    ...
    def _to_module_path(py_file: Path) -> Optional[str]:
        ...
        (the package root).  Falls back to searching for ``ravi``
        in the path parts.
        """
        parts = py_file.resolve().parts
        # Find the *last* 'ravi' in the path to anchor at src/ravi (not the
        ...
        # Build dotted path from ravi/... down to filename (sans .py)
