...
``_anyio_to_thread.run_sync`` for zero-copy reads on platforms where
...
import anyio.to_thread as _anyio_to_thread  # type: ignore[attr-defined]
...
class LocalFileStore(FileStore):
    ...
    async def startup(self) -> None:
        await _anyio_to_thread.run_sync(
    ...
    async def put(
        ...
        *,
        ...
        await _anyio_to_thread.run_sync(path.write_bytes, blob)
    ...
    async def get(self, key: str) -> bytes:
        ...
        return await _anyio_to_thread.run_sync(path.read_bytes)
    ...
    async def get_stream(
        ...
        async def _iter() -> AsyncIterator[bytes]:
            fd = await _anyio_to_thread.run_sync(lambda: open(path, "rb"))
            try:
                while True:
                    chunk = await _anyio_to_thread.run_sync(fd.read, chunk_size)
                    if not chunk:
                        break
                    yield chunk
            finally:
                await _anyio_to_thread.run_sync(fd.close)
    ...
    async def exists(self, key: str) -> bool:
        path = self._abs(key)
        return await _anyio_to_thread.run_sync(path.is_file)
    ...
    async def head(self, key: str) -> FileRef:
        ...
        stat = await _anyio_to_thread.run_sync(path.stat)
        blob = await _anyio_to_thread.run_sync(path.read_bytes)
    ...
    async def delete(self, key: str) -> None:
        path = self._abs(key)
        if path.is_file():
            await _anyio_to_thread.run_sync(path.unlink)
    ...
    async def delete_prefix(self, prefix: str) -> int:
        ...
        if target.is_file():
            await _anyio_to_thread.run_sync(target.unlink)
            return 1
        ...
        for root, _dirs, files in os.walk(target):
            for f in files:
                fp = Path(root) / f
                await _anyio_to_thread.run_sync(fp.unlink)
                count += 1
        # clean up empty dirs
        await _anyio_to_thread.run_sync(shutil.rmtree, target, True)
        return count
    ...
    async def copy(self, src_key: str, dst_key: str) -> FileRef:
        ...
        await _anyio_to_thread.run_sync(shutil.copy2, src, dst)
