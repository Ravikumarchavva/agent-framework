...
Usage::
    ...
    from ravi.core.pipelines import PipelineRunner, PipelineConfig
...
from ravi.core.agents.flow import BaseFlow, FlowStep, ParallelFlow, SequentialFlow
...
from ravi.core.pipelines.middleware import BaseWorkflowMiddleware, WorkflowRunnable
...
logger = logging.getLogger("ravi.pipelines.runner")
    ...
    async def _build_condition_pipeline(
        ...
        *,
        ...
        upstream_agents = [
            ...
            if config.node_by_id(e.source) is not None
            and config.node_by_id(e.source).node_type == NodeType.AGENT  # type: ignore[union-attr]
        ]
    ...
    async def _build_agent(
        ...
        *,
        ...
        approval_nodes = [
            ...
            for e in config.edges_from(agent_node.id)
            if config.node_by_id(e.target) is not None
            and config.node_by_id(e.target).node_type == NodeType.APPROVAL  # type: ignore[union-attr]
        ]
        if approval_nodes and hitl_bridge is not None:
            try:
                ...
                tools = [*tools, AskHumanTool(handler=CLIHumanHandler())]
                first_approval = approval_nodes[0]
                logger.info(
                    "Injected AskHumanTool for approval node %s",
                    first_approval.id if first_approval else "unknown",
                )
        ...
        if model_context is None:
            from ravi.core.context.implementations import (
        ...
        if agent_model != getattr(model_client, "model", None):
            ...
            from ravi.integrations.llm.openai.openai_client import (
                OpenAIClient,
            )
