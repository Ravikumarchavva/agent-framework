...
class AgentCatalog:
    ...
    def register(self, spec: ResourceSpec, instance: Any) -> "AgentCatalog":
        """Register a resource. Raises ``ValueError`` on FQN collision."""
        fqn = spec.fqn
    ...
    def register_tool(
        ...
        *,
        ...
        fqn = spec.fqn
        if fqn in self._resources:
            logger.warning("AgentCatalog: tool '%s' already registered — skipping", fqn)
            return self
    ...
    def register_memory(
        ...
        *,
        ...
        if spec.fqn in self._resources:
            return self
    ...
    def init_skills(
        ...
        """Initialise the embedded SkillManager for filesystem skill discovery."""
...
# ``from ravi.core.catalog.registry import AgentCatalogRegistry``
