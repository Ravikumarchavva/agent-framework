...
Example::
    ...
    from ravi.core.structured import parse, ClassificationResult
...
from ravi.core.messages.client_messages import SystemMessage
...
async def parse(
    ...
    *,
    ...
    ``client.generate(response_format=...)`` with an optional system-message
    prepend and type parameters.

    Args:
        client: Any ``BaseModelClient`` implementation. The client must
            support ``generate(response_format=...)``.
