"""Generic middleware execution engine — kernel infrastructure.

A dependency-free, fully generic before/execute/after/on_error runner. Agent
middleware (reasoning), workflow middleware (orchestration), and the kernel's
own ``MiddlewarePipeline`` all share this one ordering and error semantics.
Pure infrastructure: imports only stdlib, so it belongs in the L0 kernel.
"""
