...
# ---------------------------------------------------------------------------
# Activation / lease errors
# ---------------------------------------------------------------------------


class LeaseAcquisitionFailed(Exception):
    """Raised when this worker cannot acquire the lease for an agent.

    ``current_holder_worker_id`` names the worker that owns the contested
    lease. Callers can use this to forward the envelope to the holding
    ...
    def __init__(
        self,
        agent_id: str,
        current_holder_worker_id: str = "",
        message: str = "",
    ) -> None:
        self.agent_id = agent_id
        self.current_holder_worker_id = current_holder_worker_id
        super().__init__(
            message
            or f"lease for {agent_id!r} held by {current_holder_worker_id or '<unknown>'}"
        )
