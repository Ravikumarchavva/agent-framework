...
class ToolRisk(str, Enum):
    ...
    SENSITIVE = "sensitive"
    CRITICAL = "critical"
    UNKNOWN = "unknown"  # sentinel — means "inherit from class default"

    @property
    def color(self) -> str:
        return {"safe": "green", "sensitive": "yellow", "critical": "red"}.get(self.value, "grey")
...
# ---------------------------------------------------------------------------
# Marker protocols  (structural typing so the kernel never name-checks features)
# ---------------------------------------------------------------------------


@runtime_checkable
class ResettableTool(Protocol):
    """A tool that holds per-run state and exposes ``reset()`` to clear it.

    The agent loop calls ``reset()`` between runs on any tool that matches
    this protocol — no class import needed. Concrete tools that need this
    behaviour (e.g. ``AskHumanTool`` in ``ravi.catalog``) simply define a
    ``reset()`` method; the kernel never references the class.
    """
...
class ToolResult(BaseModel):
    ...
    ``is_error`` is the canonical attribute name (PEP 8 snake_case).
    Serialization methods (``to_mcp_format``, etc.) map to ``isError`` where
    the MCP wire format requires it.
    """
    ...
    data_ref: Optional[Any] = Field(
        default=None,
        description="DataRef pointer for large output — avoids context bloat",
    )
...
class BaseTool(ABC):
    ...
    def __init__(
        ...
        hitl_mode: HitlMode = HitlMode.UNKNOWN,
        ...
        self.risk: ToolRisk = risk if risk is not ToolRisk.UNKNOWN else type(self).risk
        # HITL mode: instance overrides class default if explicitly provided
        self.hitl_mode: HitlMode = (
            hitl_mode if hitl_mode is not HitlMode.UNKNOWN else type(self).hitl_mode
        )
        self.hitl_timeout_seconds: Optional[float] = (  # type: ignore[misc]
    ...
    def _validate_input(self, kwargs: Dict[str, Any]) -> None:
        ...
        try:
            ...
            _jsonschema.validate(instance=kwargs, schema=self.input_schema)
        except ImportError:
            ...
            if _jsonschema is not None and isinstance(exc, _jsonschema.ValidationError):
                raise ValueError(
                    f"Input validation failed for tool '{self.name}': {exc.message}"  # type: ignore[union-attr]
                ) from exc
            raise
    ...
    def get_schema(self) -> Tool:
        ...
        return Tool(
            ...
            meta=getattr(self, "_meta", None),  # type: ignore[call-arg]
