"""Restate worker entry-point — ``python -m ravi.integrations.runtime.restate.worker``.
...
def main() -> None:
    """Entry-point: ``python -m ravi.integrations.runtime.restate.worker``."""
    import argparse
    ...
    uvicorn.run(
        "ravi.integrations.runtime.restate.app:app",
        host=args.host,
        port=args.port,
        log_level="info",
    )
