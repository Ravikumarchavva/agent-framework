...
class ContentFilterMiddleware:
    ...
    async def process(self, context: AgentRunContext, call_next: Callable[[], Awaitable[None]]) -> None:
        ...
        for kw in self.blocked_keywords:
            if kw in text_lower:
                raise MiddlewareTermination(f"ContentFilter: Blocked keyword detected: '{kw}'")
                
        for pattern in self._compiled:
            match = pattern.search(text)
            if match:
                raise MiddlewareTermination(f"ContentFilter: Blocked pattern matched: '{pattern.pattern}'")
