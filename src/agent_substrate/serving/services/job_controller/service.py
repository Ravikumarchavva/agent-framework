...
from agent_substrate.serving.services.job_controller.models import JobRun
from agent_substrate.integrations.events import EventBus
from agent_substrate.serving.shared.events.types import workflow_failed, workflow_started
    ...
    *,
    ...
    try:
        ...
        async with session_factory() as db:
            started_run = await start_run(db, run.id)
            if started_run is None:
                raise RuntimeError(f"Run {run_id} could not be started")
            run = started_run
            await db.commit()
