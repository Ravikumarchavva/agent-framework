...
Entry point: uvicorn agent_substrate.serving.services.tool_executor.app:app --port 8015
"""
...
from agent_substrate.serving.services.base import create_service_app
from agent_substrate.serving.services.tool_executor.executor import ToolRegistry
from agent_substrate.serving.services.tool_executor.routes import router
from agent_substrate.serving.shared.events.factory import get_event_bus
...
def _load_default_tools(ci_http_client=None) -> list:
    ...
    if ci_http_client is not None:
        try:
            from agent_substrate.capabilities.tools.code_interpreter.tool import (
                CodeInterpreterTool,
            )
...
async def lifespan(app):
    ...
    if ci_url:
        try:
            from agent_substrate.capabilities.tools.code_interpreter.http_client import (
