...
4. **Cancellation cascade.**
   ``cancel(handle)`` appends a cancel intent to the parent log and delivers a
   cancel wakeup (``Wakeup(kind="signal", signal="cancel", ...)``?) to each
   direct child via the Scheduler, which delivers to their children, recursively.
   A child mid-effect honours the cancel at its next ``ctx.check()``; the
   at-most-once effect guarantee still holds.
...
from agent_substrate.kernel.messaging.message import Message, Payload
...
class RunHandle(BaseModel):
    ...
    Returned by ``Supervisor.spawn``; passed to ``join``/``cancel``.
